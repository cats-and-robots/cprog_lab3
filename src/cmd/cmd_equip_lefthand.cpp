/*
 * cmd_equip_lefthand.cpp
 *
 *  Created on: 3 mar 2014
 *      Author: findus
 */

#include "cmd_equip_lefthand.hpp"

equip_lefthand::equip_lefthand(){}

equip_lefthand::~equip_lefthand(){}

std::pair< p_P, p_R > equip_lefthand::execute(p_P P, p_R R, std::string com = ""){
	if (P->isEquipped_LH()){
		std::cout<<"You have already equipped your left hand!"<<std::endl;
	...
	if (nr_of_weapons>0){
		...
		std::cout<<"Equipped <"<<new_weapon->type()<<"> "<<new_weapon->name()<<" to "<<P->name()<<"'s left hand."<<std::endl;
		P->equip_LH(std::move(new_weapon));
	}
	else
